...
function createElseForExceptions(remaining, funcDef, state) {
    if (remaining.length > 0) {
        ...
        return ts.createBlock([
            utils_2.createMethodCallStatement(identifiers_1.COMMON_IDENTIFIERS.console, 'error', [
                ts.createLiteral(`Unexpected exception while handling ${funcDef.name.value}: `),
                identifiers_1.COMMON_IDENTIFIERS.err,
            ]),
            utils_2.createConstStatement(identifiers_1.COMMON_IDENTIFIERS.result, ts.createTypeReferenceNode(identifiers_1.THRIFT_IDENTIFIERS.TApplicationException, undefined), utils_2.createApplicationException('UNKNOWN', ts.createLiteral(messages_1.MESSAGES.unexpectedException))),
            utils_2.createMethodCallStatement(identifiers_1.COMMON_IDENTIFIERS.output, 'writeMessageBegin', [
                ts.createLiteral(funcDef.name.value),
